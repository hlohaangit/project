...
class linkedList {
    ...
    /**
     * @description : Prints content to file
    ...
    printToFile(filepath){
        ...
        var iterator = this.head;
        while(iterator != null){
            tempString += '{"' + iterator.data + '"},'
            iterator = iterator.next;
        }
        var resultString = tempString.substring(0,tempString.length-1);
        resultString+="]";
        try{
            fileReaderWriter.writeFileSync(filepath,resultString);
        }catch(error){
